...
describe('Delete user: ', function () {
  before(async () => {
    ...
    await utils.createAgent('TempUser1122', 'tempuser1122@test.com', 'password', true);
  ...
  it('Delete a user when an admin clicks the delete button', async () => {
    ...
    assert.equal(
      await users.getModalMessage(),
      `Are you sure you want to remove the user 'TempUser1122', this action is irreversible?`
    );
