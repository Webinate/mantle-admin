...
describe('Finds users by username and email: ', function () {
  before(async () => {
    let agent = await utils.refreshAdminToken();
    joe = await utils.createAgent('Joe', 'joe222@test.com', 'password');
    await utils.createAgent('Joey', 'joey444@test.com', 'password');
    await utils.createAgent('Mary', 'mary333@test.com', 'password');
    await utils.createAgent('Unactivated', 'unactivated333@test.com', 'password', true);
