...
export default class UsersPage extends Page {
  ...
  async getUserByEmail(email: string) {
    ...
    for (const user of users) {
      const text = await user
        .executionContext()
        .evaluate((elm) => elm.querySelector('.mt-user-email').textContent, user);
      if (text === email) return user;
    }
  ...
  async closeModal(cancel = true) {
    ...
    if (cancel) await this.click('.mt-users-modal .mt-cancel');
    else await this.click('.mt-users-modal .mt-confirm');
  ...
  clickFilterBtn() {
    return this.page.click(this.$filterSearch);
  }
}
