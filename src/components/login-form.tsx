...
export default class LoginForm extends React.Component<Props, State> {
  ...
  private onLogin() {
    ...
    if (this.state.user !== '' && this.state.pass !== '') this.props.onLogin(this.state.user, this.state.pass);
  }
   ...
   */
  getUserError() {
    if (this.state.retrievePassError) return 'Please specify a username';
    if (this.state.formSubmitted && !this.state.user) return 'Please specify a username';
  ...
  render() {
    return (
      <form className="login-form" action="" name="login">
        <FormControl className="mt-username" fullWidth={true} error={this.getUserError() ? true : false}>
        ...
        <ButtonsDiv>
          ...
          <AnchorBtnsDiv>
            <Link className="mt-create-account" to="/register">
              Create an Account
            </Link>{' '}
            |
            <AnchorBtns
