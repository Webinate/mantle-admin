...
export class MediaNavigator extends React.Component<Props, State> {
  ...
  renderRenameForm() {
    return (
      ...
      >
        ...
        <DialogActions>
          <Button id="mt-media-cancel-btn" onClick={() => this.setState({ showRenameForm: false })} color="primary">
            Cancel
          </Button>
          <Button
            variant="contained"
  ...
  private onDelete() {
    ...
    if (volumePage) {
      ...
      else this.setState({ deleteMessage: `Are you sure you want to delete these [${numToDelete}] volumes?` });
    } else {
      ...
      else this.setState({ deleteMessage: `Are you sure you want to delete these [${numToDelete}] files?` });
    }
  }
  ...
  render() {
    ...
    if (!this.props.activeVolumeId && mediaSelected) collapsed = 'none';
    else if (this.props.activeVolumeId) collapsed = 'none';
