...
export default class NewVolumeForm extends React.Component<Props, State> {
  ...
  render() {
    ...
    if (this.props.volume.memoryAllocated === 0) {
      memError = true;
      memElm = <FormHelperText id="mt-volume-memory-error">Allocated memory cannot be 0</FormHelperText>;
    } else if (this.props.volume.memoryAllocated! > this.props.maxValue) {
      memError = true;
      memElm = (
        <FormHelperText id="mt-volume-memory-error">
          <b>{formatBytes(this.props.volume.memoryAllocated!)}</b> - Allocated memory cannot be greater than{' '}
          {formatBytes(this.props.maxValue)}
        </FormHelperText>
      );
    } else {
      memElm = (
    ...
    return (
      <Container>
        <div>
          <FormControl error={!this.props.volume.name ? true : false}>
        ...
        <div>
          <FormControl disabled={!this.props.isAdmin} error={memError}>
