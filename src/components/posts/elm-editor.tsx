...
export class ElmEditor extends React.Component<Props, State> {
  ...
  componentWillReceiveProps(next: Props) {
    ...
    if (selected.length > 0) {
      ...
      if (index !== -1) this._lastActiveIndex = index;
    }
  ...
  private onWindowKeyDown(e: KeyboardEvent) {
    ...
    if (this._canPaste && e.ctrlKey && e.keyCode === 67) {
      if (this.props.selected.length > 0 && this.props.focussedId === '') this.copyElementsToLocalStorage(true);
      else localStorage.setItem('elm-editor-clip', '');
    }
    // CTRL + x
    ...
    // Escape
      ...
      if (selected) this.updateElmHtml(selected, null, 'deselect');
    }
  }

  private getLastLeafNode(source: Node) {
    if (source.childNodes.length === 0) return source;
  ...
  private getFirstEditable(source: Node) {
    ...
    function getFirstEditableChild(child: Node | null): Node | null {
      ...
      if (canContainText(child)) return child;
  ...
  private updateElmHtml(
    ...
    if (!this._createEditor || !this._activeElm) {
      if (deselect) this.props.onSelectionChanged([], false);
      return;
    }
  ...
  private focusLast(el: HTMLElement) {
    if (this._lastFocussedElm) this._lastFocussedElm.classList.remove('cursor');
    ...
    if (!this.elementInViewport(el)) el.scrollIntoView();
  ...
  private activateElm(elm: HTMLElement) {
    ...
    if (this._createEditor && this._firstElm) this.focusLast(this._firstElm);
  }
  ...
  private onKeyDown(e: React.KeyboardEvent<HTMLElement>) {
    ...
    // Tab
    if (e.keyCode === 9) inline = e.shiftKey ? 'outdent' : 'indent';
    else if (e.ctrlKey && e.keyCode === 66) inline = 'bold';
    else if (e.ctrlKey && e.keyCode === 73) inline = 'italic';
    else if (e.ctrlKey && e.keyCode === 85) inline = 'underline';
    // Enter
    else if (e.keyCode === 13) return this.onEnter(e);
  ...
  private renderAnchorModal() {
    return (
      ...
      >
        <DialogContent>
          <DialogContentText>Enter a URL in the field below</DialogContentText>
          <TextField
            autoFocus
      ...
      <div>
        {zones.map((z, index) => (
          <Tab
            ...
            className={`mt-editor-tab ${selectedZone === z ? 'active' : 'inactive'}`}
          >
            {z}
          </Tab>
        ))}
        <Container>
          <EditorToolbar
            onCreateBlock={(type, html) =>
              this.props.onCreateElm([{ type, html, zone: this.state.selectedZone }], this._lastActiveIndex + 1)
            }
...
const Container = styled.div`
  ...
  border-radius: 4px;
  box-shadow: 0px 1px 2px 0px rgba(0, 0, 0, 0.2), 0px 1px 1px 0px rgba(0, 0, 0, 0.14),
